 ...
 */
interface IERC726 {    
    /* struct Account is used for record an account information for specific purpose 
    /**
     * @notice Define a struct Account, used for recording an account information for specific
     *         purpose
     ...
     *  - balance:  balance of the account, its representation needs to use 'decimals' as explained
                    above.
     *  - _contract: address of contract for the asset, it does not apply when it asset is ether.
