...
## Abstract
This is a standard to manage operating cash flow in community. The set of smart contract interfaces defines how the operating cash are running in the a community where NFT tokens are taken as ID Passes. The operating cash can only flow by following the rules pre-set up in the contract implementation, this openess ensures the confidence of the community members, i.e. the NFT holders.

Interfaces mainly include: 1) creating and setting up multiple account for different purpose, 2) registering/binding asset inflow source address and asset outflow receuver address to a particular account, 3) inflowing (receiving in) and outflowing (sending out) asset (ether and non-ether asset such as ERC20 or ERC721 Tokerns), 4) NFT royaltiy distribution, 5) dividend(benifits sourced from the community, to be distributed to entitled NFT holders) distribution, 6) enquirying the net value and accumulated dividend distributed of each NFT token. 

As community ID Pass, NFT is EIP-721 compatibable, which can be either complete Non-fungile or semi-fungible.
  
## Motivation
Fast development of NFT market in 2021 is sharply having shrunk, due to lack of effective ecology system of NFT communities. There is no operating mechanism to create sustainable value for NFT holders, fund raised by offering NFT is not open to the NFT holders in traditional NFT markets. This EIP defines a frame and structure for setting the rules for the all the fund (operating cash), including NFT offering proceeding and other income sources in the NFT community, to flow in and out.    
...
Asset inflow sources and outflow receivers need to be binded (registered) to a certain account to for asset inflowing and outflowing to be recorded to the right account, and more importantly, this can enchance the security of the asset inflow.
  
In addition to address-account binding, more detailed and specific rules need to be implemented for asset outflowing, e.g. pre-set conditions of duration and amout must be met for asset outflowing. Amount and benificiaries must be determined in a immutable way for royalty distribution and divided distribution.
...
Special attention should be paid to ensure that the asset (both ether and non-ether assets) inflow/outflow functions not be hackered with either amount or address. Signature-verification or Merkle-proof may be implemented to enhance the security. 
